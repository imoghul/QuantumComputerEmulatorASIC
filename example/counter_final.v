...
module counter ( clock, in, latch, dec, zero );
  ...
  DFF_X1 value_reg_0_ ( .D(n20), .CK(clock), .Q(value[0]), .QN(n32) );
  DFF_X1 value_reg_2_ ( .D(n18), .CK(clock), .Q(value[2]), .QN(n33) );
  SDFF_X1 value_reg_1_ ( .D(1'b1), .SI(n35), .SE(n36), .CK(clock), .Q(value[1]) );
  DFF_X1 value_reg_3_ ( .D(n34), .CK(clock), .QN(value[3]) );
